...
JsonEx.getDataASync = function(showId, machine, afterSuccess, afterFailed){
    ...
    jQuery.ajax({
        ...
        success: function(response){
            ...
            {
                // 如果返回消息表示token已过期
                if((response.code == 1) && (response.message.indexOf("token不存在或者已过期") >= 0))
                {
                    $toastMask("身份信息过期，请重新登录验证！");
                    // 强制登录
...
/*    
    $.getJSON(url, function(response){
		...
		{
            // 如果返回消息表示token已过期
            if((response.code == 1) && (response.message.indexOf("token") >= 0))
            {
                // 强制登录
                login();
            }
...
JsonEx.post = function(machine, params, afterSuccess, afterFailed)
{
    ...
    jQuery.ajax({
        ...
        success: function(json){
            // 如果返回消息表示token已过期
            if((json.code == 1) && (json.message.indexOf("token") >= 0))
            {
                $toastMask("身份信息过期，请重新登录验证！");
                // 强制登录
                window.setTimeout(function(){
